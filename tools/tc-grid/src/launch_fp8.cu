#include "tc_grid.h"
#include "fp8_kernels.cuh"
#include "fp8_postscale_kernels.cuh"
...
LaunchResult launch_fp8(const LaunchSpec & s, const void * d_W,
    ...
    if (BK % 16) { R.note = "BK constraints"; return R; }
    if (K % QK_F8) { R.note = "K not multiple of 128 (F8 block)"; return R; }
    ...
    if (s.version == 20) { R.note = "v10s only INT8 (use version=30 for F8 SplitK)"; return R; }
    ...
    size_t smem_bytes;
    if      (s.version == 30) smem_bytes = smem_bytes_opt_v3;  // v3s reuses v3 SMEM
    ...
    const uint8_t * Wp = (const uint8_t *) d_W;

#define LAUNCH_V5(b_m, b_n, b_k, w, fm, fn)                                     \
    if (s.version==5 && BM==(b_m) && BN==(b_n) && BK==(b_k) && WARPS==(w) && s.frag_n==(fn)) { \
        kernels::fp8_v5::mm_f8_e4m3_b128_lut_v5<(b_m),(b_n),(b_k),(w),(fm),(fn)> \
            <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K, num_tile_x, num_tile_y); \
        goto done;                                                              \
    }
...
#define LAUNCH_V4(b_m, b_n, b_k, w, fm, fn)                                     \
    if (s.version==4 && BM==(b_m) && BN==(b_n) && BK==(b_k) && WARPS==(w) && s.frag_n==(fn)) { \
        kernels::fp8_v4::mm_f8_e4m3_b128_lut_v4<(b_m),(b_n),(b_k),(w),(fm),(fn)> \
            <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K);           \
        goto done;                                                              \
    }
...
#define LAUNCH_V3_B1(b_m, b_n, b_k, w, fm, fn)                                  \
    if (s.version==3 && BM==(b_m) && BN==(b_n) && BK==(b_k) && WARPS==(w) && s.frag_n==(fn)) { \
        kernels::fp8_v3::mm_f8_e4m3_b128_lut_v3<(b_m),(b_n),(b_k),(w),(fm),(fn)> \
            <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K);           \
        goto done;                                                              \
    }
...
#undef LAUNCH_V3
    ...
    // v3s (F8 SplitK). version=30. grid.z = ks.
#define LAUNCH_V3S(b_m, b_n, b_k, w, fm, fn, ks)                                \
    if (s.version==30 && BM==(b_m) && BN==(b_n) && BK==(b_k) && WARPS==(w) && s.frag_n==(fn) && s.split_k==(ks)) { \
        ...
        kernels::fp8_v3s::mm_f8_e4m3_b128_lut_v3s<(b_m),(b_n),(b_k),(w),(fm),(fn),(ks)> \
            <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K);           \
        goto done;                                                              \
    }
...
#define LAUNCH_V2(b_m, b_n, b_k, w, fm, fn)                                     \
    if (s.version==2 && BM==(b_m) && BN==(b_n) && BK==(b_k) && WARPS==(w) && s.frag_n==(fn)) { \
        kernels::fp8_opt::mm_f8_e4m3_b128_lut_v2<(b_m),(b_n),(b_k),(w),(fm),(fn)> \
            <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K);           \
        goto done;                                                              \
    }
...
#define DISPATCH(b_m, b_n, b_k, w)                                              \
    if (BM==(b_m) && BN==(b_n) && BK==(b_k) && WARPS==(w)) {                    \
        if (s.path == UnpackPath::LUT)                                          \
            kernels::fp8::mm_f8_e4m3_b128_lut<(b_m),(b_n),(b_k),(w)>            \
                <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K);       \
        else                                                                    \
            kernels::fp8_b::mm_f8_e4m3_b128_post_scale<(b_m),(b_n),(b_k),(w)>   \
                <<<grid, block, smem_bytes>>>(Wp, d_act, d_dst, M, N, K);       \
        goto done;                                                              \
    }
    DISPATCH(16, 64, 32, 4)
...
#define RERUN(b_m, b_n, b_k, w)                                                 \
    ...
    size_t bytes_w = (size_t) N * ((size_t) K / QK_F8) * (1 + QK_F8);
